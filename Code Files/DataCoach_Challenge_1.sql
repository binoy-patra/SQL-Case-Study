...
Steve's Car Showroom (CHALLENGE 1)
Link: https://mattsteel87.wixsite.com/datacoach/blank-page-1
...
TABLES
Here are the tables you will be using: Cars, Sales, SalesPerson (Data is also Provided)
*/
-- ----------------------------------------------------------------------------------------------------------------------------------------
...
FROM
    DATACOACH.CARS C
        JOIN
    DATACOACH.SALES S ON C.CAR_ID = S.CAR_ID
WHERE
    YEAR(S.PURCHASE_DATE) = 2022;

-- 2. What is the total number of cars sold by each salesperson?
SELECT 
    SP.SALESMAN_ID AS ID,
    SP.NAME AS NAME,
    COUNT(S.SALE_ID) AS TOTAL_CAR_SOLD
FROM
    DATACOACH.SALESPERSONS SP
        JOIN
    DATACOACH.SALES S ON SP.SALESMAN_ID = S.SALESMAN_ID
GROUP BY SP.SALESMAN_ID , SP.NAME;

-- 3. What is the total revenue generated by each salesperson?
SELECT 
    SP.SALESMAN_ID, SP.NAME, SUM(C.COST_$) Total_Revenue
FROM
    DATACOACH.SALESPERSONS SP
        JOIN
    DATACOACH.SALES S ON SP.SALESMAN_ID = S.SALESMAN_ID
        JOIN
    DATACOACH.CARS C ON S.CAR_ID = C.CAR_ID
GROUP BY SP.SALESMAN_ID , SP.NAME;

-- 4. What are the details of the cars sold by each salesperson?
SELECT 
    SP.salesman_id AS ID, SP.name AS NAME, c.*
FROM
    datacoach.salespersons SP
        JOIN
    datacoach.sales S ON SP.salesman_id = S.salesman_id
...
SELECT 
    sp.salesman_id, c.*, s.purchase_date
FROM
    datacoach.cars c
        JOIN
    datacoach.sales s ON c.car_id = s.car_id
        JOIN
    datacoach.salespersons sp ON s.salesman_id = sp.salesman_id
WHERE
    sp.name = 'Emily Wong'
...
SELECT 
    sp.name, sp.city, COUNT(s.sale_id) AS Sales
FROM
    datacoach.salespersons sp
        JOIN
    datacoach.sales s ON sp.salesman_id = s.salesman_id
...
SELECT 
    sp.name, sp.age, SUM(c.cost_$) AS Revenue
FROM
    datacoach.salespersons sp
        JOIN
    datacoach.sales s ON sp.salesman_id = s.salesman_id
